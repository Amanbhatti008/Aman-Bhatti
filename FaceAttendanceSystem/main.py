...
class AttendanceSystem(QWidget):
    ...
    def setupCamera(self):
        ...
        if not self.cap.isOpened():
            self.status_label.setText("❌ Camera not detected!")
            return
    ...
    def loadEncodings(self):
        try:
            with open(os.path.join(BASE_DIR, "EncodeFile.p"), "rb") as f:
                self.encodeListKnown, self.studentIds = pickle.load(f)
        except Exception as e:
            self.status_label.setText(f"❌ Error loading encodings: {e}")
            return

        self.recognized_students = {}
        try:
            self.detector = dlib.get_frontal_face_detector()
            self.predictor = dlib.shape_predictor(os.path.join(BASE_DIR, "shape_predictor_68_face_landmarks.dat"))
        except Exception as e:
            self.status_label.setText(f"❌ Error loading dlib models: {e}")
            return
    ...
    def get_attendance_summary(self, student_id):
        ...
        try:
            if os.path.exists(self.attendance_file_path):
                with open(self.attendance_file_path, "r", encoding='utf-8') as f:
    ...
    def go_home(self):
        try:
            ...
            self.close()
            # Note: HomePage import commented out as it's not provided
    ...
    def mark_attendance(self, student_info):
        ...
        try:
            ...
            with open(self.attendance_file_path, "a", newline="", encoding='utf-8') as f:
                writer = csv.writer(f)
            ...
            self.attendance_status_label.setText(f"❌ Error marking attendance: {e}")
            return
    ...
    def update_frame(self):
        ...
        if self.last_matched_id:
            ...
            if student_info:
                ...
                student_info = {k: v for k, v in student_info.items() if 'image' not in k.lower()}
